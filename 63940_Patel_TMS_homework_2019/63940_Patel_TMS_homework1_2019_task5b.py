...
def plot_trajectories(times,velocities):
    fig, ax = plt.subplots()
    times = np.divide(times,1e-9)
    ax.plot(velocities,times[1:],'-o',markevery = [0])
    ax.plot(velocities,times[1:],'D',markevery = [-1])
    ...
    fig.savefig('Task_5b.png')
    plt.close(fig)

plot_trajectories(times,velocities)